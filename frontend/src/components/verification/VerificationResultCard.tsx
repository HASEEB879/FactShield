import { BrainCircuit, CheckCircle2, Link2, Quote, ShieldCheck } from "lucide-react";
        ...
        <section className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(260px,0.8fr)] lg:items-end">
          <div>
            <div className="mb-3 flex items-center justify-between gap-4 text-sm">
              <span className="font-medium text-slate-200">Confidence meter</span>
        ...
        <section className="rounded-2xl border border-violet-300/10 bg-violet-300/[0.035] p-5 sm:p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400">Why this verdict was returned</p>
            </div>
          </div>
          <p className="mt-5 leading-8 text-slate-200">{result.explanation}</p>
        </section>

        <section>
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <h3 className="text-lg font-semibold text-white">Evidence sources</h3>
              <p className="mt-1 text-sm text-slate-400">
                Sources referenced in this verification response
              </p>
            </div>
          ...
          {result.sources.length > 0 ? (
            <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
              {result.sources.map((source, index) => (
                <div
                  key={`${source}-${index}`}
                  className="group rounded-2xl border border-white/10 bg-white/[0.03] p-4 transition hover:-translate-y-0.5 hover:border-cyan-300/30 hover:bg-cyan-300/[0.05]"
                >
